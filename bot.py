...
def build_keyboard(links: dict, show_all: bool, key: str, album_buttons: list[tuple[str, str]]) -> InlineKeyboardMarkup:
    ...
    # 1) Botones de plataforma (canción/álbum/artista indistinto)
    fila = []
    ...
    # 2) Sección de Álbum (solo si hay)
    if album_buttons:
...
def _guess_kind_from_url(url: str) -> str | None:
    """
    Heurística rápida por si Odesli no trae type:
    - '/artist/' en Apple/Spotify => 'artist'
    - '/album/' => 'album'
    - '/track/' o detalle de video => 'song'
    """
    p = urlparse(url)
    path = p.path.lower()
    host = p.netloc.lower()
...
async def fetch_odesli(url: str):
    """
    Devuelve (links_by_platform, title, artist, cover_url, entity_type)
    entity_type: 'song' | 'album' | 'artist' | None
    """
    ...
    try:
        ...
        # tipo de entidad (si viene)
        ent_type = (entity.get("type") or "").lower() or None
        # heurística por si falta
        if not ent_type:
            ent_type = _guess_kind_from_url(url)

        # Para artistas, Odesli suele poner el nombre en 'title' y 'artistName' vacío
        title = entity.get("title")
        artist = entity.get("artistName")
        thumb = entity.get("thumbnailUrl")

        return links_for_track or None, title, artist, thumb, ent_type
    except Exception as e:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for url in urls:
        ...
        links, title, artist, cover, ent_type = await fetch_odesli(url)
        if not links:
            continue

        # Solo intentamos derivar álbumes para tracks (y a veces YT/SC)
        album_buttons = []
        if ent_type not in ("artist",):
        ...
        # Caption según tipo
        if ent_type == "artist":
            name = title or artist or "Artista"
            caption = f"👤 {name}\n🔗 Disponible en:"
        else:
...
async def _post_init(app: Application):
    # limpiar cualquier webhook previo
    try:
